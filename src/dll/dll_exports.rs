...
pub unsafe extern "stdcall" fn f_D3DPERF_SetOptions(dw_options: u32) {
	...
	unsafe { crate::dll::dll_imports::D3DPERF_SetOptions(dw_options) }
}

#[unsafe(no_mangle)]
pub unsafe extern "stdcall" fn f_Direct3DCreate9(sdk_version: u32) -> *mut IDirect3D9 {
	let r = unsafe { crate::dll::dll_imports::Direct3DCreate9(sdk_version) };
...
pub unsafe extern "stdcall" fn f_Direct3DCreate9Ex(
	...
	let r = unsafe { crate::dll::dll_imports::Direct3DCreate9Ex(sdk_version, pp) };
	log::trace!(
		"f_Direct3DCreate9Ex(sdk_version: (UINT) {sdk_version:?}, pp: (*mut *mut IDirect3D9Ex) {pp:?}) -> HRESULT {r:?}"
	);
	r
}
