...
type FnPresent = unsafe extern "system" fn(this: IDirect3DDevice9, psourcerect: *const RECT, pdestrect: *const RECT, hdestwindowoverride: HWND, pdirtyregion: *const RGNDATA) -> HRESULT;

static mut FN_ORG: Option<FnEndScene> = None;
static mut FN_ORG_PRESENT: Option<FnPresent> = None;

unsafe extern "system" fn HOOK_EndScene(this: IDirect3DDevice9) -> HRESULT {
    let fn_EndScene = FN_ORG.unwrap();
    let r = fn_EndScene(this);
    r
}

unsafe extern "system" fn HOOK_Present(this: IDirect3DDevice9, psourcerect: *const RECT, pdestrect: *const RECT, hdestwindowoverride: HWND, pdirtyregion: *const RGNDATA) -> HRESULT {
    let fn_Present = FN_ORG_PRESENT.unwrap();
    let r = fn_Present(this, psourcerect, pdestrect, hdestwindowoverride, pdirtyregion);
    //trace!("Present!");
    r
}
...
impl IDirect3D9_Impl for MyD3D9 {
    ...
    fn GetAdapterDisplayMode(
        ...
        info!("GetAdapterDisplayMode_pre");
        let r = unsafe { self.f.GetAdapterDisplayMode(adapter, pmode) };
        info!("GetAdapterDisplayMode");
        r
    }
    ...
    fn CheckDeviceMultiSampleType(
        ...
        info!("CheckDeviceMultiSampleType_pre");
        let r = unsafe {
        ...
        info!("CheckDeviceMultiSampleType");
        r
    }
    ...
    fn CheckDepthStencilMatch(
        ...
        info!("CheckDepthStencilMatch_pre");
        let r = unsafe {
    ...
    fn GetAdapterMonitor(&self, adapter: u32) -> windows::Win32::Graphics::Gdi::HMONITOR {
        info!("GetAdapterMonitor_pre");
        let r = unsafe { self.f.GetAdapterMonitor(adapter) };
        info!("GetAdapterMonitor");
        r
    }
    ...
    fn CreateDevice(
        ...
        let dev: &IDirect3DDevice9 = unsafe { (*ppreturneddeviceinterface).as_ref().unwrap() };
        set_hook(&dev);
        set_hook_present(&dev);

        // TODO: instead of hooking EndScene here, create a fake IDirect3DDevice9:
        //use crate::dll::f_direct3d9device::*;
        //**ppreturneddeviceinterface = Some(MyDirect3DDevice9::new(dev))?;
        Ok(r)
    }
}
...
fn set_hook(dev: &IDirect3DDevice9) {
    ...
    unsafe {
        FN_ORG = Some(std::mem::transmute(*fn_saved));
    }
